...
public class ProjectorPlaneEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        if (Application.isPlaying && GUILayout.Button("Projector"))
...
public class ProjectorPlane : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (CreateOnStart)
            CreateMesh();
        mc = GameObject.FindObjectOfType<MeshCollider>();
    }

    private void OnDestroy()
    {
        if (Mesh)
            DeepCore.Unity3D.UnityHelper.Destroy(Mesh);
    }
    ...
    private bool UpdateMeshVerties()
    {
        ...
        for (int i = 0; i < vertCount; i++)
        {
            ...
            vertWorldPos.y = mc.bounds.max.y;
            ray.origin = vertWorldPos;
            ray.direction = Vector3.down;

            if (mc.Raycast(ray, out hit, mc.bounds.size.y))
            {
