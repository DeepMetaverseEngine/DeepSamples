...
namespace Slate{
	...
	public class AnimatorTrack : CutsceneTrack {
		...
		protected const int ROOTMOTION_FRAMERATE = 30;
		...
		protected Dictionary<PlayAnimatorClip, int> ports;
		[HideInInspector]
        protected int activeClips;
		...
		protected bool useBakedRootMotion;
        protected List<Vector3> rmPositions;
        protected List<Quaternion> rmRotations;
		...
		public virtual void UpdateClip(PlayAnimatorClip playAnimClip, float clipTime, float clipPrevious, float weight){
		...
		public virtual void DisableClip(PlayAnimatorClip playAnimClip){
		...
		public virtual void CreateAndPlayTree(){
#if UNITY_2017_1_OR_NEWER
			...
			var basePlayableClip = AnimationClipPlayable.Create(GetNewAnimationClip(baseAnimationClip));
            ...
            foreach (var playAnimClip in actions.OfType<PlayAnimatorClip>()){
                ...
                var playableClip = AnimationClipPlayable.Create(GetNewAnimationClip(playAnimClip.animationClip));
        ...
        private AnimationClip GetNewAnimationClip(AnimationClip clip)
        {
            if (animator != null&& animator.runtimeAnimatorController != null && clip != null)
            {
                ...
                Debug.Log("AnimationclipName = [" + clip.name + "] is null");
            }
            return clip;
        }

		//The root motion must be baked if required.
		public void BakeRootMotion(){
