...
public class CustomTBDragOrbit : MonoBehaviour
{
	...
	void Awake()
	{
		camera_me.transform.position = cameraFar.transform.position;
		camera_me.transform.localEulerAngles = cameraFar.transform.localEulerAngles;
	}
    ...
    void Apply()
    {
        ...
        if( smoothMotion )
        {
			...
			transform.Rotate(new Vector3(1, 0, 0), (pitch - tempPitch)/360.0f);
			transform.Rotate(new Vector3(0, 1, 0), (yaw - tempYaw)/360.0f);
        }
        else
        {
...
//        	camera.gameObject.transform.position = ( target.position + panOffset ) - (distance) * camera.gameObject.transform.forward;
			...
			camera_me.transform.localEulerAngles = farAngle +
