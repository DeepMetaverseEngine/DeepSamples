...
namespace DeepCore.Unity3D
{
    ...
    {
        public delegate void AfterRemoveDelegate(V val);

        private AfterRemoveDelegate OnAfterRemove = null;

        private Dictionary<K, int> refcount = new Dictionary<K, int>();

        public LRUCache(int capacity, AfterRemoveDelegate function)
        {
        ...
        public bool Add(K key, V val)
        {
            ...
            if (cacheMap.TryGetValue(key, out tmp))
            {
                foreach (LinkedListNode<LRUCacheItem<K, V>> a in tmp)
                {
                    ...
                    {
                        // UnityEngine.Debug.LogError("[LRU] Add: false");
                        return false;
                    }
                }
            }
            ...
            // UnityEngine.Debug.LogError("[LRU] cap cnt: "+ lruList.Count +" "+ capacity);
            ...
            //UnityEngine.Debug.LogError("[LRU] Add: true");
            return true;
        }
        ...
        public bool ContainsKey(K key)
        {
            ...
            while (iter.MoveNext())
            {
                LinkedListNode<LRUCacheItem<K, V>> node = iter.Current;
                V value = node.Value.value;
                if (value == null || (value is UnityEngine.Object && !UnityHelper.IsObjectExist(value as UnityEngine.Object)))
                {
        ...
        protected void removeFirst()
        {
            ...
            if (node != null)
            {
                ...
                if (this.OnAfterRemove != null) this.OnAfterRemove(node.Value.value);
            }
        }
