    ...
    public class HZUnityAssetBundleManager : MonoBehaviour
    {
        ...
        public void RemoveAssetRef(string refName)
        {
            ...
            if (assetRef.TryGetValue(refName, out refCount))
            {
                ...
                if (newCount == 0)
                {
                    UnloadAssetBundle(refName, false);
                }
            }
            else
            {
                Debug.LogWarning("RemoveRef error: " + refName);
            }
        }

        public void CheckAssetRef(string refName)
        {
            int refCount;
            if (!assetRef.TryGetValue(refName, out refCount) || refCount == 0)
            {
                UnloadAssetBundle(refName, false);
            }
        }
