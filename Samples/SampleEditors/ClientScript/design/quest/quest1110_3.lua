...
--	api.Unit.AddBubbleTalk(30001,"這是個NPC測試泡泡","0",2000)
	...
	--api.Unit.AddBubbleTalk(30033,"過了橋就是我家了","0",4)
      local params = {
     	duration = 10,
     	nodeparams = "30028|ef_buff_heal", --①模板id為0就是玩家自己反之就是unit模板id ②掛指定模板 后面沒有節點名字 就默認走腳底,節點注意大小寫，
 	 }
     api.PlaySceneEffect(30028,"ef_buff_heal",params)
	 	 --動作序列播放 當前視野范圍內的模板   參數-1 播放一次  0-循環
 	 local action = {
      	{"c_pa",1},
      	{"n_idle",0},
 	 }
 	-- api.Unit.ChangeUnitPlayAnim(30028,action)
	 
	 api.Unit.ChangeUnitPlayAnim(30028,'n_idle')
	-- api.Sleep(1)
	api.Unit.AddBubbleTalk(30028,"謝謝你救了我！","0",4)
