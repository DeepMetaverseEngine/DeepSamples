...
--	api.Unit.AddBubbleTalk(30001,"這是個NPC測試泡泡","0",2000)
	...
	--api.PauseBGM()
     --播放場景特效
      --local params = {
      	--pos = {55.66,162.6},  --場景坐標
      	--duration = -1,
 	 --}
    

	--api.Unit.AddBubbleTalk(0,"點擊后就能開啟傳送門","0",4)
	--api.Sleep(2)
	--api.PlaySceneEffect(1,"ef_portal_08",params)--①自定義key要唯一  ②特效名字
	--api.ResumeBGM()
	api.Unit.ChangeUnitPlayAnim(24302,'f_dead')	
	local objid = api.Unit.AddUnit(30003,57,38.96,2.7)
	local objid = api.Unit.AddUnit(30001,55.91,40.90,4.5)
end