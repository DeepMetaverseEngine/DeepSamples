...
--	api.Unit.AddBubbleTalk(30001,"這是個NPC測試泡泡","0",2000)
    ...
    --播放cg
     ...
     --播放場景特效
      local params1 = {
      	pos = {97.13,129.24},  --場景坐標
		duration = 99999,
		direct = 1.36   --角度(弧度)
 	 }
      local params2 = {
      	pos = {65.85,123.22},  --場景坐標
		duration = 99999,
		direct = 1.36   --角度(弧度)
 	 }	 
      local params3 = {
      	pos = {42.06,70.01},  --場景坐標
		duration = 99999,
		direct = 1.36   --角度(弧度)
 	 }    
      local params4 = {
      	pos = {100.42,89.17},  --場景坐標
		duration = 99999,
		direct = 1.36   --角度(弧度)
 	 }	
...
--  api.PlaySceneEffect(1002,"ef_portal_08",params2)--①自定義key要唯一  ②特效名字
--  api.PlaySceneEffect(1003,"ef_portal_08",params3)--①自定義key要唯一  ②特效名字
--  api.PlaySceneEffect(1004,"ef_portal_08",params4)--①自定義key要唯一  ②特效名字
	api.RemoveEffectByKey(1001)
	api.RemoveEffectByKey(1002)
	api.RemoveEffectByKey(1003)
	api.RemoveEffectByKey(1004)
end