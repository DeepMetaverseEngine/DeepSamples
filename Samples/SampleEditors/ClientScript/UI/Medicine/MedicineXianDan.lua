...
function _M.OnEnter(self)
	...
	local function UpdateElement(node, index)
		--item图标Canvas
		local itemCvs = node:FindChildByEditName('cvs_item', true)
		local itemID = itemIndex[index]
		local itemCount = itemCountInBag[itemID]
		local itemLbCount = node:FindChildByEditName('lb_num', true)
		local itemLbName = node:FindChildByEditName('lb_name', true)
		local itemLbDesc = node:FindChildByEditName('lb_action', true)
		...
		print('DataItemID = ',d.itemID,'ItemID = ',itemID)
		...
		node.TouchClick = function()
			...
			if canUse == false then
				--Tips
				GameAlertManager.Instance:ShowNotify(Util.GetText('medicine_nolevel'))
				return 
			end
...
function _M.SetCurUseItem(self, itemID)
	...
	MedicineModel.SaveMedicineItemID(itemID)
	--self.itemID = itemID
