...
function DataHelper.GetVersion(tb_name)
    if tb_name == '_luaversion_' then
        return LocalVersions['version']
    end
    local tbpath = DataTag[tb_name] or tb_name
    assert(type(tbpath) == 'string')
    return LocalVersions[tbpath]
end

local function FixVersionKeys(data)
    local ret = {}
    for k, v in pairs(data) do
        local name = string.gsub(k, '%.lua$', '')
...
local function CheckInitDB()
    if not db then
        ...
        LocalVersions = FixVersionKeys(versions)
    -- print_r('LocalVersions',LocalVersions)
    end
end

function DataHelper.CompareVersion(versions)
    versions = FixVersionKeys(versions)
    local notsame = {}
    for k, v in pairs(versions) do
        if k ~= 'version' and v ~= LocalVersions[k] then
            notsame[k] = v
        end
    end
    return notsame
...
function DataHelper.SetDataTable(tb_name, data, version)
    if tb_name == '_luaversion_' then
        local notsame = {}
        data = FixVersionKeys(data)
        for k, v in pairs(data) do
            if k ~= 'version' and v ~= LocalVersions[k] then
                table.insert(notsame, k)
            end
        end
        LocalVersions = data
        return notsame
...
function DataHelper.FindFirst(tb_name, find_key)
    ...
    local tkey = type(find_key) 
    if tkey == 'table' or tkey == 'function' then
