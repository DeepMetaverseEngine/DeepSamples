...
local AccordionExt = {}
DisplayUtil.warpOOPSelf(AccordionExt)
...
function AccordionExt.new(scrollPan, menuCanvas, menuBtnName, itemBtnName, selectFunc, clickLockFunc, isMutexItemMenu, autoSelectFirstItem, isHorizontal, animate,fixtopgap)
    local obj = {}
    setmetatable(obj, AccordionExt)
    obj:_initArgs(scrollPan, menuCanvas, menuBtnName, itemBtnName, selectFunc, clickLockFunc, isMutexItemMenu, autoSelectFirstItem, isHorizontal, animate,fixtopgap)
    obj:_init()
    return obj
end
...
function AccordionExt:setData(list)
    ...
    for i,v in ipairs(list) do
        ...
        if not menuData then
            ...
            -- print("self.i",i)
            -- print_r("self._menuList",self._menuList)
        end
...
function AccordionExt:_onItemClick(sender)
    ...
    local menuIdx = math.floor(sender.UserTag / INDEX_WEIGHT)
    local data = self._list[menuIdx]
    -- print("_onItemClick", sender.UserTag, menuIdx, itemIdx, self._selectMenuIdx, self._selectItemIdx)
    -- print(PrintTable(self._list))
    local menuData = self._menuList[menuIdx]
...
function AccordionExt:_openMenu(menuData, tween)
    ...
    self._moving = false--not not tween

    local y = menuData.menuNode[self._PH] + self._fixtopgap
    local originY = y 
...
function AccordionExt:_initGap()
    ...
    -- if self._isHorizontal then
    --     self._itemGap = itemNode.X - menuNode.X - menuNode.Width
    --     self._lastItemGap = self._menuCanvas.Width - itemNode.X - itemNode.Width
    -- else
    --     self._itemGap = itemNode.Y - menuNode.Y - menuNode.Height
    --     self._lastItemGap = self._menuCanvas.Height - itemNode.Y - itemNode.Height
    -- end
end

local function getFirstChildName(root, deepChildName)
    local child = root:FindChildByEditName(deepChildName, true)
    while not GameUtil.EqualsObj(root, child.Parent) do
        child = child.Parent
    end
    return child.EditName
end

function AccordionExt:_initArgs(scrollPan, menuCanvas, menuBtnName, itemBtnName, selectFunc, clickLockFunc, isMutexItemMenu, autoSelectFirstItem, isHorizontal, animate,fixtopgap)
    local args = {
        scrollPan = scrollPan,
...
function AccordionExt:_init()
    ...
    -- <{node:menuCanvas, menuNode:MenuNode, idx, menuBtn, itemBtns:<ItemBtn>,itemNodes:<ItemNode>, isOpen:bool, idx:int}>
    self._menuList = {}
...
function AccordionExt.onInitMenuNode(self, node, btn, data)
    node.IsGray = not not data.isLock
    btn.Text = data.name
    btn.IsChecked = false
    if self.onRefreshMenu ~= AccordionExt.onRefreshNode then
        self:onRefreshMenu(node, btn, data)
    end
end
function AccordionExt.onInitItemNode(self, node, btn, data)
    node.IsGray = not not data.isLock
    btn.Text = data.name
    btn.IsChecked = false
    if self.onRefreshItem ~= AccordionExt.onRefreshNode then
        self:onRefreshItem(node, btn, data)
    end
end
