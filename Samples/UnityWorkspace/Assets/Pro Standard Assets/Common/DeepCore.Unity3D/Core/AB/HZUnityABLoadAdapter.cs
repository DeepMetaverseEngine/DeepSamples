...
namespace DeepCore.Unity3D
{
    ...
    public class HZUnityABLoadAdapter
    {
        ...
        public virtual void SetFinishCallBack(LoadFinishCallBack callBack)
        {
            OnFinshCallBack = callBack;
        }
        public virtual void AddCallBack(HZUnityLoadAdapterCallBack callBack)
        {
            OnLoadCallBack += callBack;
        }
        ...
        public HZUnityAssetBundle GetHZUnityAssetBundle()
        {
            if (mHZUnityAssetBundle == null)
            {
                ...
                if(ab != null)
                {
                    mHZUnityAssetBundle = new HZUnityAssetBundle(mURL, ab, mVersionCount++);
                }
            }
